...
import { Values } from './extends/values';

type FormatFactory = (data: DataPublicPluginStart) => (mapping?: SerializedFieldFormat) => IFieldFormat;

type FormatFactoryInstance = (data: DataPublicPluginStart) => FormatFactory;

const getFormatInstance: FormatFactoryInstance = (data) => (mapping?) => {
  return data.fieldFormats.deserialize(mapping as any);
};

let getFormat!: FormatFactory;

export class Component extends ReactComponent {

  _getLabels() {
    const config = (this.props as any).visParams.metric;
  ...
  _processTableGroups(table) {
    const config = (this.props as any).visParams.metric;
    const dimensions = (this.props as any).visParams.dimensions;
    const isPercentageMode = config.percentageMode;
  ...
  _filterBucket = (metric) => {
    ...
    const table = (this.props as any).visData;
    (this.props as any).vis.API.events.filter({table, column: dimensions.bucket.accessor, row: metric.rowIndex});
  };
  ...
  _renderMetric = (metric, index) => {
    return (
      <Values
        key={index}
        metric={metric}
        fontSize={(this.props as any).visParams.metric.style.fontSize}
        onFilter={(this.props as any).visParams.dimensions.bucket ? this._filterBucket : null}
        showLabel={(this.props as any).visParams.metric.labels.show}
      />
    );
  };
  ...
  render() {
    if (!getFormat) {
      getFormat = getFormatInstance((this.props as any).vis.type.visConfig.dependencies.data as any);
    }
    let metricsHtml;
    if ((this.props as any).visData) {
      let metrics = this._processTableGroups((this.props as any).visData);
      const formulas = (this.props as any).visParams.metric.formulas;
    ...
    return (<div style={{
      ...
      width: '100%',
      height: '100%',
    }} className="extendedMtrVis">{metricsHtml}</div>);
  }

  componentDidMount() {
    (this.props as any).renderComplete();
  }
